...
contract EthRegistrarSubdomainRegistrar is AbstractSubdomainRegistrar {
    ...
    function configureDomainFor(string memory name, uint price, uint referralFeePPM, address payable _owner, address _transfer) public {
        ...
        bytes32 label = keccak256(bytes(name));
        require(owner(label) == address(this));
	...
	*/
     ...
     */
    function register(bytes32 label, string calldata subdomain, address _subdomainOwner, address payable referrer, address resolver, bytes calldata contenthash) external not_stopped payable {
        ...
        doRegistration(domainNode, subdomainLabel, subdomainOwner, Resolver(resolver), contenthash);
